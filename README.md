...
- **Per-display configuration** - each monitor gets its own wallpaper, remembered by hardware ID across unplug / replug
- **Smart idle detection** - global mouse + keyboard monitoring; a display without input for N minutes (default 5, configurable) automatically starts playing its dynamic wallpaper
...
3. Open the menu bar panel → Settings, and pick a wallpaper for each display (system wallpaper, video, or image sequence).

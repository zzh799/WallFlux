...
- **媒体感知闲置** - 其他应用正在输出声音时（Chrome / Safari 网页视频、直播、播放器、音乐等），播放所在显示器保持活跃、不进入闲置循环播放，壁纸不会覆盖你正在看的内容；声音停止后恢复闲置检测。通过系统 CoreAudio 公开 API 检测，无需任何权限；「媒体应用」设置页预置国内外常见音乐应用白名单（真实播放过声音后自动忽略，听歌时壁纸照常循环，点右下角「默认忽略应用」查看名单），同时展示本机播放过声音的应用及最近播放时间，可逐项忽略指定应用（被忽略的应用即使正在出声也不会阻止闲置循环播放）

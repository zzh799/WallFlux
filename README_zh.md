...
- **微跳防烧屏** - 活跃显示器不创建壁纸窗口，当前帧直接设置为系统壁纸；每 Y 秒向前跳 Z 帧并直接修改系统壁纸，视觉上静止不动，却能有效防止 OLED 烧屏
...
- **屏保级置顶播放** - 播放期间壁纸窗口置于最顶层（屏保层级），类似屏保效果；鼠标进入时立即让位（暂停并隐藏窗口，露出桌面），不遮挡工作窗口
...
- **媒体感知闲置** - 其他应用正在输出声音时（Chrome / Safari 网页视频、直播、播放器、音乐等），播放所在显示器保持活跃、不进入闲置循环播放，壁纸不会覆盖你正在看的内容；声音停止后恢复闲置检测。通过系统 CoreAudio 公开 API 检测，无需任何权限；「媒体应用」设置页预置国内外常见音乐应用白名单（真实播放过声音后自动忽略，听歌时壁纸照常循环，点右下角「默认忽略应用」查看名单），同时展示本机播放过声音的应用及最近播放时间（正在出声的置顶并标注所在显示器），可逐项忽略指定应用（被忽略的应用即使正在出声也不会阻止闲置循环播放）
...
- **短暂进入宽限**（默认 5 秒，可在设置中调整）：鼠标进入播放中的显示器时壁纸立即让位（暂停并隐藏壁纸窗口，露出桌面）并暂停，宽限期内鼠标移出或停止移动则恢复置顶播放；点击/滚动/键盘等强交互立即退出。
- **微跳模式**（活跃态）不创建壁纸窗口：当前帧直接设为**系统壁纸**，每 Y 秒向前跳 Z 帧并直接修改系统壁纸：画面看似静止，实际不会长时间停留同一像素，实现防烧屏。退出 WallFlux 时恢复原系统壁纸。
- 壁纸窗口仅在**闲置循环播放时置于顶层**（屏保层级 `kCGScreenSaverWindowLevel`）可见，其余时间不显示任何窗口；跟随所有 Spaces，且不拦截鼠标事件。
...
- **壁纸不显示** - 活跃态壁纸通过系统壁纸 API 直接设置，不依赖窗口层级；若桌面不显示，请确认系统设置「桌面与屏幕保护程序」未被其他应用覆盖，并重启 WallFlux 重新设置。
